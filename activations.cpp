...
public:
	...
	void init(std::string name = "FC", int input_dim = 0, int output_dim = 0, float init_scale = 0){
		...
		params.insert(std::pair<std::string, std::vector<std::vector<T> > >(w_name, util_scalar_mult(init_scale, util_random_matrix(input_dim, output_dim))));
		params.insert(std::pair<std::string, std::vector<std::vector<T> >>(b_name, util_scalar_mult(init_scale, util_zero_matrix(output_dim))));
	...
	std::vector<std::vector<T> > forward(std::vector<std::vector<T> > feat){
		std::vector<std::vector<T> > output = NULL;
		try{
	...
	std::vector<std::vector<T> > backward(std::vector<std::vector<T> > dprev){
		...
		std::vector<std::vector<T> > dfeat = NULL;
		grads.insert(std::pair<std::string, std::vector<std::vector<T> > >(w_name, NULL));
		grads.insert(std::pair<std::string, std::vector<std::vector<T> > >(b_name, NULL));
		grads.insert(std::pair<std::string, std::vector<std::vector<T> > >(b_name, util_sum(dprev, 0)));
		dfeat = util_mult(dprev, util_reshape(util_transpose(params[w_name]), meta.size(), meta[0].size()));
		grads.insert(std::pair<std::string, std::vector<std::vector<T> > >(w_name, util_mult(util_transpose(util_reshape(feat, 1, input_dim)),dprev)));
		meta = NULL;
		return dfeat;
	};
