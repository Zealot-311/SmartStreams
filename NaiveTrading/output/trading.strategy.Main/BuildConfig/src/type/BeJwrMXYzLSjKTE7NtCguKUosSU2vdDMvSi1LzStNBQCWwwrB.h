// t3F5pricei8strategyF7revenue


#ifndef BEJWRMXYZLSJKTE7NTCGUKUOSSU2VDDMVSI1LZSTNBQCWWWRB_H_
#define BEJWRMXYZLSJKTE7NTCGUKUOSSU2VDDMVSI1LZSTNBQCWWWRB_H_
...
#define SELF BeJwrMXYzLSjKTE7NtCguKUosSU2vdDMvSi1LzStNBQCWwwrB
...
public:
    ...
    typedef SPL::float64 price_type;
    typedef SPL::int32 strategy_type;
    typedef SPL::float64 revenue_type;

    enum { num_attributes = 3 } ;
    
    SELF() : Tuple(), price_(), strategy_(), revenue_() {}
    SELF(const Self & ot) : Tuple(), price_(ot.price_), strategy_(ot.strategy_), revenue_(ot.revenue_) 
      { constructPayload(ot); }
    SELF(const price_type & _price, const strategy_type & _strategy, const revenue_type & _revenue) : Tuple(), price_(_price), strategy_(_strategy), revenue_(_revenue) { }
    ...
    strategy_type & get_strategy() { return strategy_; }
    const strategy_type & get_strategy() const { return strategy_; }
    void set_strategy(const strategy_type & _strategy) { strategy_ = _strategy; }
    
    revenue_type & get_revenue() { return revenue_; }
    const revenue_type & get_revenue() const { return revenue_; }
    void set_revenue(const revenue_type & _revenue) { revenue_ = _revenue; }
    ...
    size_t hashCode() const
    {
        ...
        s = 37 * s + std::tr1::hash<strategy_type >()(strategy_);
        s = 37 * s + std::tr1::hash<revenue_type >()(revenue_);
        return s;
    }
    
    size_t getSerializedSize() const
    {
        size_t size = sizeof(SPL::float64)+sizeof(SPL::int32)+sizeof(SPL::float64);
    ...
    { 
        price_ = ot.price_;
        strategy_ = ot.strategy_;
        revenue_ = ot.revenue_; 
        assignPayload(ot);
        return *this; 
    }
    ...
    { 
        ...
        std::swap(strategy_, ot.strategy_);
        std::swap(revenue_, ot.revenue_);
...
#undef SELF
#endif // BEJWRMXYZLSJKTE7NTCGUKUOSSU2VDDMVSI1LZSTNBQCWWWRB_H_ 
