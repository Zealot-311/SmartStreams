...
#include "BeJwrMXIzLSjKTE7NtCguKUosSU2vBABE8gdJ.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<float64 price,int32 strategy>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    
    {
        ...
        TypeOffset t(MY_OFFSETOF(strategy_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "strategy") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, strategy_);
   else
     istr >> strategy_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, strategy_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "strategy=", get_strategy()) ;
    ostr << '}';
}
