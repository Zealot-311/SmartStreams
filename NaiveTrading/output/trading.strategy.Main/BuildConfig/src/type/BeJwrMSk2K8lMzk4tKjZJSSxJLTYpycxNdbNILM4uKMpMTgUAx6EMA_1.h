// t4s6tickers4dates4timeF8askprice


#ifndef BEJWRMSK2K8LMZK4TKJZJSSXJLTYPYCXNDBNILM4UKMPMTGUAX6EMA_1_H_
#define BEJWRMSK2K8LMZK4TKJZJSSXJLTYPYCXNDBNILM4UKMPMTGUAX6EMA_1_H_
...
#define SELF BeJwrMSk2K8lMzk4tKjZJSSxJLTYpycxNdbNILM4uKMpMTgUAx6EMA_1
...
public:
    ...
    typedef SPL::rstring ticker_type;
    typedef SPL::rstring date_type;
    typedef SPL::rstring time_type;
    typedef SPL::float64 askprice_type;

    enum { num_attributes = 4 } ;
    
    SELF() : Tuple(), ticker_(), date_(), time_(), askprice_() {}
    SELF(const Self & ot) : Tuple(), ticker_(ot.ticker_), date_(ot.date_), time_(ot.time_), askprice_(ot.askprice_) 
      { constructPayload(ot); }
    SELF(const ticker_type & _ticker, const date_type & _date, const time_type & _time, const askprice_type & _askprice) : Tuple(), ticker_(_ticker), date_(_date), time_(_time), askprice_(_askprice) { }
    ...
    ticker_type & get_ticker() { return ticker_; }
    const ticker_type & get_ticker() const { return ticker_; }
    void set_ticker(const ticker_type & _ticker) { ticker_ = _ticker; }
    ...
    askprice_type & get_askprice() { return askprice_; }
    const askprice_type & get_askprice() const { return askprice_; }
    void set_askprice(const askprice_type & _askprice) { askprice_ = _askprice; }
    ...
    {
        buf << ticker_ << date_ << time_ << askprice_;
    }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<ticker_type >()(ticker_);
        s = 37 * s + std::tr1::hash<date_type >()(date_);
        s = 37 * s + std::tr1::hash<time_type >()(time_);
        s = 37 * s + std::tr1::hash<askprice_type >()(askprice_);
        return s;
    }
    ...
    size_t getSerializedSize() const
    {
        size_t size = sizeof(SPL::float64);
           size += ticker_.getSerializedSize();
   size += date_.getSerializedSize();
   size += time_.getSerializedSize();
    ...
    void swap(SELF & ot) 
    { 
        std::swap(ticker_, ot.ticker_);
        std::swap(date_, ot.date_);
        std::swap(time_, ot.time_);
        std::swap(askprice_, ot.askprice_);
...
#undef SELF
#endif // BEJWRMSK2K8LMZK4TKJZJSSXJLTYPYCXNDBNILM4UKMPMTGUAX6EMA_1_H_ 
